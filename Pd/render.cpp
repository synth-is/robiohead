...
/*
*  MCT4054 MODIFICATION GLOBAL
*  ------------
*/

#define RAD_TO_DEG 57.295779513082325

AuxiliaryTask taskLSM9DS1;
void readLSM9DS1(void* imu_p);

int readCount = 0;
int readIntervalSamples = 0;

const int LSM9DS1acc = 1; // 1 ENABLE - 0 DISABLE - ACCELEROMETER
const int LSM9DS1gyr = 1; // 1 ENABLE - 0 DISABLE - GYROSCOPE
const int LSM9DS1cmp = 1; // 1 ENABLE - 0 DISABLE - COMPASS
const int LSM9DS1fps = 1; // 1 ENABLE - 0 DISABLE - FUSION POSE
...
if ((imu == NULL) || (imu->IMUType() == RTIMU_TYPE_NULL)) {
	fprintf(stderr, "Error: RTIMUlib No IMU found\n");
	return false;	
}
...
readIntervalSamples = (int)floor((float)imu->IMUGetPollInterval() * 0.001 * context->audioSampleRate);

taskLSM9DS1 = Bela_createAuxiliaryTask(readLSM9DS1, 50, "bela-LSM9DS1", imu);
...
void render(BelaContext *context, void *userData)
{
	...
	for(unsigned int tick = 0; tick < numberOfPdBlocksToProcess; ++tick)
	{
		...
		/*
		*  MCT4054 MODIFICATION LOOP
		*  ------------
		*/
		if(++readCount >= readIntervalSamples) {
	    	readCount = 0;
	        Bela_scheduleAuxiliaryTask(taskLSM9DS1);
		}
		/*********/
...
/*
*  MCT4054 MODIFICATION Auxiliary task to read LSM9DS1
*  ------------
*/
void readLSM9DS1(void* imu_p)
{
	RTIMU *imu = (RTIMU *) imu_p;
    ...
    while (imu->IMURead()) {
	    ...
	    if(LSM9DS1acc == 1){
	    	if (!libpd_start_message(3)) {
		        libpd_add_float(imuData.accel.x());
		        libpd_add_float(imuData.accel.y());
	    	    libpd_add_float(imuData.accel.z());
	        	libpd_finish_list("LSM9DS1acc");
	        }
	    }
	    if(LSM9DS1gyr == 1){
			if (!libpd_start_message(3)) {
		        libpd_add_float(imuData.gyro.x());
		        libpd_add_float(imuData.gyro.y());
		        libpd_add_float(imuData.gyro.z());
		        libpd_finish_list("LSM9DS1gyr");
			}
	    }
	    if(LSM9DS1cmp == 1){
	    	if (!libpd_start_message(3)) {
		        libpd_add_float(imuData.compass.x());
		        libpd_add_float(imuData.compass.y());
		        libpd_add_float(imuData.compass.z());
		        libpd_finish_list("LSM9DS1cmp");
	    	}
	    }
	    if(LSM9DS1fps == 1){
			if (!libpd_start_message(3)) {
		        libpd_add_float(imuData.fusionPose.x()*RAD_TO_DEG);
		        libpd_add_float(imuData.fusionPose.y()*RAD_TO_DEG);
		        libpd_add_float(imuData.fusionPose.z()*RAD_TO_DEG);
		        libpd_finish_list("LSM9DS1fps");
			}
	    }  
    }
}
